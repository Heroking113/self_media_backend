...
class TopicManage(models.Model):
    ...
    title = models.CharField(max_length=1024, verbose_name='帖子标题', default='', null=True, blank=True)
    content = models.TextField(verbose_name='帖子内容', default='')
    is_top = models.BooleanField(verbose_name='是否置顶', default=False)
    topic_type = models.CharField(verbose_name='帖子类型', max_length=8, default='0', choices=TOPIC_TYPE, null=True, blank=True, db_index=True)
    ...
    view_ids = models.TextField(verbose_name='浏览用户的id', help_text='浏览用户的id(避免uid占用过带宽)', default='', null=True, blank=True)
    liker_ids = models.TextField(verbose_name='点赞用户的id', help_text='浏览用户的id(避免uid占用过带宽)', default='', null=True, blank=True)
...
class CommentManage(models.Model):
    ...
    content = models.CharField(verbose_name='评论内容', max_length=1024, default='')
